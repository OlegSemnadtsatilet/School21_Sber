...
/*   Updated: 2022/03/15 11:33:47 by bmaya            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_texture(t_general general, int width, int height, char *path)
{
	...
	mlx_put_image_to_window(general.mlx, general.window.ptr, \
		general.texture.img, width, height);
	mlx_destroy_image(general.mlx, general.texture.img);
}

static void	determ_and_draw_texture(t_general general, \
	char simbol, int width, int height)
{
	if (simbol == '1')
		...
		draw_texture(general, width, height, "images/eisenhorn_2.xpm");
	else if (simbol == 'C')
		draw_texture(general, width, height, "images/collectible_2.xpm");
	return ;
}

static void	draw_objects(t_general general, int height_px, int width_px)
{
	height_px = 0;
	width_px = 0;
