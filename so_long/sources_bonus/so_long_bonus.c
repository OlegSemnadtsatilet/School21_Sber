...
/*   Updated: 2022/03/15 11:50:15 by bmaya            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	check_map_simbols(t_map	map)
{
	...
	while (y < map.height)
	{
		...
		while (x < map.width)
		{
			...
			&& map.field[y][x] != 'O' && map.field[y][x] != 'E' \
			&& map.field[y][x] != 'P' && map.field[y][x] != 'C')
				return (1);
			x++;
		}
		y++;
...
int	main(int argc, char **argv)
{
	...
	draw_map(general);
	mlx_string_put(general.mlx, general.window.ptr, \
		2, 2, 0xffff00, "STEPS: 0");
	mlx_loop_hook(general.mlx, *animation, &general);
